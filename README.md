...
* **Bookend Extraction (Tier 1.5)**: Only for the highly-relevant surviving papers, a concurrent worker pool (`ThreadPoolExecutor`, 10 workers) downloads the PDF and extracts the Abstract and Conclusion using `TextChunker.extract_academic_bookends(max_chars=8000)`.
* **Triage Critic (Tier 2)**: The top surviving papers (capped at 40), now enriched with their conclusion bookends, are passed to the Evaluator Agent (Triage Critic). It performs a deep methodological evaluation and selects the absolute best 25 papers (configurable, defaulting to 25) for complete full-text extraction via PyMuPDF.
...
To run deep-dive Map-Reduce analysis without hitting model limits or regional quota walls, ResearchBot implements a multi-tiered token gating and dynamic allocation pipeline. The system enforces a rigorous target budget of **600,000 characters** (approximately 150,000 to 200,000 tokens), leaving safe headroom for massive structural topologies, prompts, and system instructions:
...
* **Semantic Bookends Compression Algorithm (Stage 2)**: For individual files exceeding **60,000 characters**, ResearchBot executes an in-memory semantic compression process (`infrastructure/TextChunker.py`). The chunker detects Abstract/Introduction and Conclusion/Limitations sections via regular expressions, extracting and joining them with a marked break (always pre-applied greedily before budget calculations to prevent the pre-chunking budget deficit bug), and falls back to a structural 30k head/tail slice if section headers are missing.
* **Protected Dynamic Bucket Allocation (Stage 3)**: Mapped into synthesis summary (`50,000` chars), web scrape (`100,000` chars), and academic open-access PDF (`450,000` chars) protected character budgets.
...
* **Global Character Safety Net (Stage 5)**: If the merged corpus exceeds **600,000 characters**, an eviction algorithm drops files lowest-priority first (scrapes first, then full-text papers, never synthesis files).
